...
function applicationResourceTool(componentOffset, name, description, resources) {
  return {
    ...
    Settings: JSON.stringify({
      ...
      resources: {
        dataLists: {
          items: resources.map(([id, title, permissions]) => ({ AppID: appId, ListID: id, ListSetID: rootId, Title: title, permissions })),
        },
      },
      Inputs: [],
      Outputs: [],
...
function makeContactWorkflow() {
  ...
  const def = {
    ...
    variables: {},
    childshapes: [
      { resourceid: startId, properties: { name: "Start" }, stencil: { id: "StartNoneEvent" }, outgoing: [{ resourceid: flow1 }], bounds: { lowerRight: { x: 120, y: 120 }, upperLeft: { x: 80, y: 80 } } },
      {
        resourceid: aiId,
        properties: {
        ...
        stencil: { id: "AI" },
        incoming: [{ resourceid: flow1 }],
        outgoing: [{ resourceid: flow2 }],
        bounds: { lowerRight: { x: 360, y: 140 }, upperLeft: { x: 200, y: 60 } },
      },
      { resourceid: endId, properties: { name: "End" }, stencil: { id: "EndNoneEvent" }, incoming: [{ resourceid: flow2 }], bounds: { lowerRight: { x: 520, y: 120 }, upperLeft: { x: 480, y: 80 } } },
      { resourceid: flow1, properties: { name: "Start to AI" }, stencil: { id: "SequenceFlow" }, source: { resourceid: startId }, target: { resourceid: aiId } },
      { resourceid: flow2, properties: { name: "AI to End" }, stencil: { id: "SequenceFlow" }, source: { resourceid: aiId }, target: { resourceid: endId } },
    ],
  };
  return {
