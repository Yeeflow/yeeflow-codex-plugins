...
Source studied: `Projects Center.yap`, read-only.
...
The newly created `New Document library` has one default Type `0` view with empty `LayoutView`. This should be accepted as an export-proven newly-created-library shape, but generated polished packages should prefer a configured view JSON.
...
`New Document library` has one `New file` form, but no `ListModel.LayoutView` mapping was present in the export.
