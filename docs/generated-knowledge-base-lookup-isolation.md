...
| `knowledge-base-lookup-v8-text4-lookup.yap` | Moved lookup to source-like `Articles.Text4`, kept blank values, and hid lookup from Articles list view | package passed with `APP_THEME_EMPTY`; graph passed; wrapper round-trip passed | runtime import pending because Chrome UI automation lost macOS assistive access |
...
The source export uses `Text3` for Feature image and `Text4` for Category lookup. The next required proof is runtime import/open testing of `Knowledge Base Lookup v8 Text4 Lookup.yap`.

## Generator Rules

- Do not add article lookup fields to Knowledge Base generated packages until a source-like `Text4` lookup package is runtime proven.
