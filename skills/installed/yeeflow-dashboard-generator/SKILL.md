...
When a dashboard uses Data Filter controls, inspect `docs/studies/data-filter-controls.md`, `docs/studies/normalized/data-filter-controls/`, and `references/filter-controls-pattern.md` first. `Sales_Management_AD.yap` export-proves dashboard Checkbox, Select, Range, Check range, Date, Relative period, Apply button, and Remove filters shapes. Filter variables live in embedded page `filterVars[]`, value-producing controls bind with `__filter_`, and downstream table/report/chart consumers use expression-token references in data filter conditions. Use Apply button only for click-apply filters and treat Remove filters as a special reset control. Search and Radio remain runtime-proven from the earlier generated dashboard baseline but were not present in this export; Hierarchy and Sorting are product-documented only until a future export/runtime pass.
