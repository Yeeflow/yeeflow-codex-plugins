...
Collection/Kanban actions runtime proof package: `docs/studies/collection-kanban-actions-runtime-proof.md` tracks the current correct-project runtime-test candidate. Do not reuse earlier wrong-project artifacts or claims. Treat runtime execution as pending until the user confirms the clean-project package import/open/action checklist.

Correct-project v2 runtime-test candidate: `generate-collection-kanban-actions-runtime-proof.mjs` now emits `/Users/Renger/Downloads/collection-kanban-actions-runtime-proof.v2.yap` from the clean `formreport-clean` project. It includes one Data List, one dashboard, Collection and Kanban controls, local `attrs.actions[]`, item-template `attrs.control_action` bindings, `__ctx_coll` / `ListDataID`, selected IDs/count temp variables, checked/unchecked icon rules, and bulk update/delete page actions. Treat v2 as pending manual runtime test until the user confirms import/open/action execution.
