...
- In the active generator workspace, use `yeeflow-expression-functions.normalized.json`, `yeeflow-expression-operators.normalized.json`, `yeeflow-expression-utils.js`, `docs/yeeflow-expression-editor-reference.md`, and `docs/yeeflow-expression-generation-rules.md` before generating calculated controls, dynamic display rules, custom validation rules, lookup filters, workflow transition conditions, request numbers, or default-value formulas.
