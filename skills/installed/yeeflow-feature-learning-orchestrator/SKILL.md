...
For Collection/Kanban action learning, use `docs/studies/collection-kanban-actions.md`, `docs/studies/normalized/collection-kanban-actions/`, and `scripts/inspect-collection-kanban-actions.mjs`. `Company Overview (2).yap` export-proves dashboard Kanban local actions for Edit item and Delete item, dashboard Collection local actions for Select Items, Edit item, Delete item, and Mark current item as Completed, item-template `attrs.control_action` bindings, current item expressions with `ctx = "__ctx_coll"` and `ListDataID`, selected IDs/count temp variables, checked/unchecked dynamic display rules, and page-level bulk update/delete actions. The screenshot adds UI-reference-backed Collection item step labels including View item, Edit item, Delete item, Update fields, and Trigger list workflow. Keep runtime execution of edit/delete/update/select/bulk operations unproven until a focused generated package tests those exact paths.

`docs/studies/collection-kanban-actions-runtime-proof.md` now records the correct-project v2 generated runtime-test candidate for Collection/Kanban local actions, current item context, item selection, selected count, and bulk update/delete. Preserve the proof boundary: pending user test until `/Users/Renger/Downloads/collection-kanban-actions-runtime-proof.v2.yap` is imported and exercised from the clean project; future claims must stay limited to tested generated package/actions only, not all Collection action step types or cross-host behavior.

Correct-project v2 continuation: when rerunning this proof after the wrong-project correction, generate `/Users/Renger/Downloads/collection-kanban-actions-runtime-proof.v2.yap` from `formreport-clean`, validate it locally, and mark runtime status as pending until the user tests that exact package. Do not carry over wrong-project artifacts, screenshots, decoded payloads, or runtime claims.
