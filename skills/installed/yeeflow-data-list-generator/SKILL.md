...
Application creation quality hardening: data-list forms generated as part of a full application must be validated with the same strict app-quality gate as dashboards. Do not accept technically importable forms that are edge-to-edge, missing planned New/Edit/View/Print surfaces, or missing planned data-bound controls. When a YAP or YAPK app is generated alongside lists, enforce the package-level no-portal rules: YAP `SimplePortal` must be `null`, YAPK `PortalInfo` must be `null`, and generated Type 103 dashboards must include `Ext2 = "{\"src\":true}"`.
