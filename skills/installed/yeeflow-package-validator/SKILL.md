...
Collection/Kanban runtime-proof validation note: `docs/studies/collection-kanban-actions-runtime-proof.md` records the current correct-project runtime-test candidate. Keep the validator gate strict on action bindings, variable references, target layouts, and update fields. Do not reuse earlier wrong-project runtime claims; the v2 package remains pending runtime proof until user-tested.

Correct-project v2 validation note: `/Users/Renger/Downloads/collection-kanban-actions-runtime-proof.v2.yap` is a fresh clean-repo runtime-test candidate. Before manual import it should report zero errors from package validation, graph validation, import-readiness, Collection/Kanban action inspection, and Kanban/Collection Dynamic inspection. Warnings are acceptable only when documented as non-blocking and not promoted to runtime proof.
