...
Data Filter validation: use `scripts/inspect-data-filter-controls.mjs` with dashboard packages and enforce the rules from `docs/studies/data-filter-controls.md`. Value-producing Data Filter bindings must resolve to `page.filterVars[]`; downstream `__filter_` expression tokens in `attrs.data.filter[]` and `exts[].attr.settings.Conditions[]` must resolve; click-apply filters must reference an existing Apply button; explicit Remove filters targets must resolve when present; unknown filter control types and unsupported variable shapes should warn first unless export evidence proves them invalid.
