...
For full application generation, run `scripts/inspect-generated-app-quality.mjs --strict-app-quality --spec <approved-spec.md>` when a UI implementation spec exists. Strict mode must fail retired/legacy dashboards, Type 103 dashboards without `Ext2.src`, non-array dashboard `LayoutInResources`, blank dashboards unless explicitly allowed by test scope, missing planned app areas, Data table columns missing `Field`, YAP `SimplePortal` values other than `null`, and YAPK no-portal `PortalInfo` values other than `null`.
