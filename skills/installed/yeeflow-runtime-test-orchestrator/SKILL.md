...
Pivot Table runtime boundary: `CRM - Customer relationship management (1).yap` is export-proven for Pivot Table controls on a Dashboard page only. `docs/studies/pivot-table-control.md` and `scripts/inspect-pivot-table-controls.mjs` document and validate the schema, but import, render, aggregation correctness, Data Filter interaction, sorting, styling fidelity, and Data List form hosting are not runtime-proven. Before claiming runtime proof, use a focused generated package with safe local data, import/open it, verify Pivot Table rendering, row/column/value aggregation, date grouping, filter-variable behavior when used, and export-back stability.
