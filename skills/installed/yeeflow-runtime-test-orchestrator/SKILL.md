...
Vendor Onboarding full UI v2 checkpoint: YAPK installed successfully and YAP imported only after the current dashboard `src` marker and no-portal null rules were enforced. Before runtime testing full generated apps, confirm every Type 103 dashboard includes `Ext2 = "{\"src\":true}"`, no generated dashboard uses the retired legacy renderer, YAP `SimplePortal` is `null`, YAPK no-portal `PortalInfo` is `null`, and strict app-quality/spec coverage validation has run. Do not use runtime testing to discover blank dashboards or obvious spec omissions that local strict validation can catch.
