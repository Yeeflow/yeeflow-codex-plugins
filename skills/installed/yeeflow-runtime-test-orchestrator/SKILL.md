...
Approval workflow task-form proof from `Workflow Actions Runtime Baseline (2)_Task forms.yap` is export-proven only. A focused runtime baseline should first prove import/open/designer/publish for submission form plus multiple task forms, task-form selection on Assignment Task nodes, copied readonly task forms, task-specific editable fields, standard Action Panel, custom buttons, and Submit form operation configuration. Execute approve/reject/reassign/add-assignee/complete only with disposable requests and safe test users. `Workflow Action Approval Test.ywf` confirms the corrected Add others custom-button binding to the `submitType="5"` Add assignee action; still verify the binding in generated packages before execution and do not claim Add assignee runtime behavior until observed.
