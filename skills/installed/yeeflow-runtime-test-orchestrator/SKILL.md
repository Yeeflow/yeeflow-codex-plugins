...
V1 manual feedback: the Dynamic Sub List rendered, but the header Grid collapsed to one column and Grid Appearance could not expand. Retest V1.2 only after it is signed from the corrected V1.1 YAPK baseline. Verify header/body alignment, hidden Sub List caption, Designer Appearance expandability for layout grids, Add another item, Duplicate, Delete, and Import. Keep action execution pending until the user confirms each click result.
