...
Form Actions Phase 1 baseline: use `docs/yeeflow-form-action-generation-rules.md` for generated front-end form logic. Button styles, button click triggers, page-load triggers, temp variables, `setvar`, and `confirm` are runtime-proven; ContentList persistence for the first focused form-action app remains pending and should use proven workflow-variable button mappings.
