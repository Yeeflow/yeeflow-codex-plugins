...
Form Actions Phase 2 update: app-level approval packages can plan Query data and Submit form actions using the export-backed structures from `docs/form-actions-phase-2-query-submit-study.md`, `docs/yeeflow-form-action-query-data-step-rules.md`, and `docs/yeeflow-form-action-submit-step-rules.md`. Query multiple can populate form list variables or temp collections; query single can map fields to workflow variables; counts can be stored in temp variables; `arraySum` can aggregate a temp query collection. The collection display function token is `JSONStringfy`; `vLookup` is label-observed only and deferred. Submit form is `type: "submit"`; Save changes uses `attrs.submitType = "3"`. Do not generate `arraySub`.
...
Form Actions baseline: use `docs/yeeflow-form-action-generation-rules.md` for generated front-end form logic. Button styles, button click triggers, page-load triggers, temp variables, `setvar`, and `confirm` are runtime-proven; Query data and Submit form are Phase 2 export-backed and require the focused Phase 2 generated runtime test before being marked fully generator-proven. ContentList persistence for the first focused form-action app remains pending and should use proven workflow-variable button mappings.
