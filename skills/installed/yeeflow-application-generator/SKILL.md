...
Form Actions Phase 2 update: app-level approval packages can use Query data and Submit form actions with the runtime-proven generated patterns from `docs/generated-form-actions-phase-2-query-submit-test-v1-baseline.md`, `docs/yeeflow-form-action-query-data-step-rules.md`, and `docs/yeeflow-form-action-submit-step-rules.md`. Query multiple can populate form list variables or temp collections; query single can map fields to workflow variables; counts can be stored in temp variables and copied to workflow variables; `arraySum` can aggregate a temp query collection. The collection display function token is `JSONStringfy`; `vLookup` is label-observed only and deferred. Submit form is `type: "submit"`; Save changes uses `attrs.submitType = "3"`. Query data filters remain runtime-sensitive because the generated Active filter was ignored by runtime. Do not generate `arraySub`.
...
Form Actions baseline: use `docs/yeeflow-form-action-generation-rules.md` for generated front-end form logic. Button styles, button click triggers, page-load triggers, temp variables, `setvar`, `confirm`, Query data multiple/single mapping, query count, temp query collection aggregation, default Submit form, Save changes, approval completion, and ContentList persistence are runtime-proven in focused generated packages. Query data filters remain partial/runtime-sensitive until the ignored Active-filter behavior is root-caused.
