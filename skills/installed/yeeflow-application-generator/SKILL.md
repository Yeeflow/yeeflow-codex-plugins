...
Application creation quality hardening after Vendor Onboarding full UI v2: v0.6.2 fixed the import foundation, but full application generation must still pass strict quality gates. Do not generate retired/legacy dashboards. Every generated Type 103 dashboard must include `Ext2 = "{\"src\":true}"`, and generated-final validation must fail missing `src`, empty/missing `Ext2`, non-array `LayoutInResources`, YAP `SimplePortal` values other than `null`, YAPK `PortalInfo` values other than `null` for no-portal packages, blank dashboards unless explicitly allowed by test scope, and Data table columns missing `Field` source bindings. Run `scripts/inspect-generated-app-quality.mjs --strict-app-quality --spec <approved-spec.md>` before returning full application packages.
