...
For no-portal YAP fallback packages generated alongside YAPK candidates, emit `ListExportResult.SimplePortal = null`. Do not emit `{}` or `[]`; product feedback confirmed `SimplePortal: {}` fails import even when schema validation can normalize it for compatibility.

Application creation quality hardening after Vendor Onboarding full UI v2: generated YAPK packages can install while still underbuilding the requested app. Before handoff, run strict full-app validation against the approved spec when available. Fail generated-final packages that use legacy dashboards, omit `Ext2.src`, use `PortalInfo: {}` or `PortalInfo: []`, use YAP fallback `SimplePortal: {}` or `[]`, return blank dashboards outside an explicit test scope, or omit Data table `Field` source bindings.
