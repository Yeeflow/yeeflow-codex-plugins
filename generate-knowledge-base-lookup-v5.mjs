...
const outputAppDefPath = "knowledge-base-lookup-v5-app-def.json";
const outputResourcePath = "knowledge-base-lookup-v5-resource.json";
const outputReportPath = "knowledge-base-lookup-v5-generation-report.json";

const oldFamily = "267";
const family = "274";
const generatedAt = "2026-05-14 00:25:00";
const appTitle = "Knowledge Base Lookup v5";
const appDescription = "Knowledge Base lookup isolation package with local Categories, Articles, and Articles-to-Categories lookup.";
...
function makeListView(layoutId, listId, title, fields) {
...
function makeList(listId, title, fields, rows) {
  ...
  return {
    ...
    Defs: fields,
    Layouts: [makeListView(layoutId, listId, `All ${title}`, fields)],
    PublicForms: [],
    RemindRules: [],
    FlowMappings: [],
...
const articleFields = [
  ...
  makeLookupField(articlesListId, `${family}0020000000001004`, "Text3", "Category", categoriesListId, rootId)
];
...
fs.writeFileSync(outputReportPath, `${JSON.stringify({
  ...
  includedPatterns: [
    ...
    "local article-to-category lookup metadata and sample values",
