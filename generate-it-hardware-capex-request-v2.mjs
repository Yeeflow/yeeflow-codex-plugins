...
const outAppPath = "it-hardware-capex-request-app-def.v2.json";
const outFormDefPath = "it-hardware-capex-request-approval-form-def.v2.json";
const outListDefPath = "it-hardware-capex-request-list-def.v2.json";
const outReportPath = "it-hardware-capex-request-generation-report.v2.json";

const family = "439";
const generatedAt = "2026-05-14 17:20:00";
const appId = 41;
...
function buildWorkflow(def) {
  ...
  const submitNode = "capx-node-request-submission-0002";
  const setNode = "capx-node-set-0003";
  const taskIds = [
    [submitNode, "Request Submission", false, def.pageurls[1].id],
    ["capx-node-manager-0004", "Line Manager Approval", false, def.pageurls[1].id],
    ["capx-node-architecture-0005", "IT Architecture Review", false, def.pageurls[1].id],
    ["capx-node-security-0006", "IT Security Review", false, def.pageurls[1].id],
    ["capx-node-finance-0007", "Finance Approval", false, def.pageurls[1].id],
    ["capx-node-procurement-review-0008", "Procurement Review", false, def.pageurls[1].id],
    ["capx-node-procurement-processing-0009", "Procurement Processing", true, def.pageurls[1].id],
    ["capx-node-goods-receipt-0010", "Goods Receipt and Asset Registration", true, def.pageurls[1].id],
    ["capx-node-requester-confirmation-0011", "Requester Confirmation", true, def.pageurls[1].id]
  ];
  const contentNode = "capx-node-contentlist-0012";
  const endNode = "capx-node-end-0013";
  const rejectNode = "capx-node-reject-0014";
  const mainFlows = Array.from({ length: 12 }, (_, index) => `capx-flow-main-${String(index + 1).padStart(4, "0")}`);
  ...
  reviewTasks.forEach(([taskId, name, complete, taskUrl], index) => {
    ...
    nodes.push(taskNode(taskId, name, taskUrl, incoming, approved, rejected, 600 + index * 220, 120, complete));
  });
  ...
  nodes.push(workflowFlow(mainFlows[1], submitNode, setNode, {
    ...
    conditioninfo: [{ key: "capx-cond-submitted", pre: "and", left: taskOutcomeButton(submitNode, "Request Submission"), op: "s.=", right: outcomeValueButton("Approved") }]
  }));
  nodes.push(workflowFlow(mainFlows[2], setNode, taskIds[1][0], { name: "Set Request No. to Line Manager Approval" }));
  reviewTasks.forEach(([taskId, name], index) => {
  ...
  nodes.push(workflowFlow(rejectFlows[0], submitNode, rejectNode, {
    ...
    documentation: "Rejected",
    conditioninfo: [{ key: "capx-cond-submit-rejected", pre: "and", left: taskOutcomeButton(submitNode, "Request Submission"), op: "s.=", right: outcomeValueButton("Rejected") }]
  }));
