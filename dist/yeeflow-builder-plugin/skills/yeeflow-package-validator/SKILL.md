...
Collection/Kanban runtime-proof validation note: `docs/studies/collection-kanban-actions-runtime-proof.md` records a generated package that passed package, graph, materialization, import-readiness, Collection/Kanban action inspection, dynamic control inspection, and wrapper round-trip checks with zero errors, then passed user runtime import/open/action testing on 2026-05-28. Keep the validator gate strict on action bindings, variable references, target layouts, and update fields. The runtime claim remains limited to the tested package and tested action set.
