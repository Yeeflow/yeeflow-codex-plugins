...
YAPK schema v2 generation rule from Vendor Onboarding v1.13: a generated `.yapk` is not a YAP wrapper. Emit top-level `AppExportPackageInfo`, set `Resource` to base64 Brotli JSON for decoded `AppPackageInfo`, and make `Childs[]` entries use `Fields` rather than YAP `Defs`. Preserve `LongAsString` fields as strings, keep `AppID = 41` where the import rules require it, use API-issued IDs for new generated package/object IDs, and validate dashboard Data table `Field` source bindings before signing.
