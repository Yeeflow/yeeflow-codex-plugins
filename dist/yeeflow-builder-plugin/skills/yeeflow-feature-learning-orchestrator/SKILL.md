...
`docs/studies/collection-kanban-actions-runtime-proof.md` now records the focused generated package for that next proof step. It is local-validation-passed and manual-test-ready for Collection/Kanban local actions, current item context, item selection, selected count, and bulk update/delete. The learning is not runtime-proven until the user imports the package and confirms the listed behaviors.
