...
V1 runtime feedback found a table-layout defect: the generated header Grid collapsed to one column and Designer Appearance could not expand. Study the user-corrected V1.1 YAPK as the layout source of truth before promotion. The corrected pattern uses a containing section, caption-off header/body grids, caption-off Sub List, and a matching grid as the first `list-body` child. V1.2 YAPK generation/signing must pass before the next manual runtime test.
