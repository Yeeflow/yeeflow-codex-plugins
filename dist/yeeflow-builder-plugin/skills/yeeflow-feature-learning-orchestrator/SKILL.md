...
`docs/studies/collection-kanban-actions-runtime-proof.md` now records the focused generated package and the user-confirmed runtime pass for Collection/Kanban local actions, current item context, item selection, selected count, and bulk update/delete. Preserve the proof boundary: tested generated package/actions only, not all Collection action step types or cross-host behavior.
