...
For Yeeflow application version-management package learning, treat `.yapk` as distinct from `.yap`. Study the wrapper first and preserve originals. Studied `.yapk` version packages are JSON with `PackageId`, `TenantID`, `AppID`, `ListID`, `Version`, `Notes`, `Author`, `Date`, `Sign`, and opaque high-entropy base64 `Resource` values without the `.yap` `[______gizp______]` gzip prefix. A metadata-only wrapper edit was rejected at runtime, and multi-version comparison showed `PackageId`, `Sign`, and `Resource` change across Yeeflow-generated versions while tenant/app/list identity stays stable. Follow-up signing evidence proves wrapper signing/verification only for already-valid `.yapk` Resource payloads: wrapper-only signed packages can be accepted but do not change app content when `Resource` is unchanged; normal `.yap` Resource encodings and single-byte Resource mutations are rejected by the signing path. Until the Resource-generation API/encoding used by Yeeflow Version management is proven, do not mutate `.yapk` wrappers or app internals for content-upgrade claims; use `.yapk` as read/inspect-only and ask product for the method that rebuilds opaque `Resource` after app-content changes.
