...
The focused generated package in `docs/studies/collection-kanban-actions-runtime-proof.md` is the current manual-test baseline for this pattern. It includes one Data List, one dashboard, Collection and Kanban controls, local item actions, current item context, selection state, checked/unchecked icons, selected count, and bulk update/delete actions. Treat it as validator-backed until the user confirms runtime action execution.
