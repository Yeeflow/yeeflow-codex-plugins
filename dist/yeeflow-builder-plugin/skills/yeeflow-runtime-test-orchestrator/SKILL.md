...
For newly generated YAP dashboards, confirm the current dashboard shell when no inline resource is present: root `Type = 103`, `LayoutView = null`, `Ext2 = "{\"src\":true}"`, and `LayoutInResources = []`. The Vendor Onboarding v1.93 export proves this current blank shell only; rich current-dashboard control content still needs a focused export/import proof.
