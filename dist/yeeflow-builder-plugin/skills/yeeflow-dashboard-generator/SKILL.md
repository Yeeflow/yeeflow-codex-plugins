...
Full application quality hardening: the `src` marker is required for every generated Type 103 dashboard, not only blank shells. Missing `Ext2 = "{\"src\":true}"` sends Yeeflow to the retired legacy renderer. Generated-final validation must fail `DASHBOARD_TYPE_103_SRC_REQUIRED`, `DASHBOARD_CURRENT_VERSION_MARKER_MISSING`, `DASHBOARD_LEGACY_RENDERER_FORBIDDEN`, and `DASHBOARD_LAYOUTINRESOURCES_INVALID`. Do not hand off a full application with blank dashboards unless the user explicitly requested an isolation/minimal dashboard test.
