...
Focused runtime candidate: `generate-kanban-collection-timeline-runtime-proof.mjs` emits a minimal dashboard app with one source Data List, Kanban, Collection, `timeline-v`, `timeline-h`, and Dynamic field/user/image/file controls all bound with source `3`. Use it to prove dashboard rendering of the shared generated pattern. Its synthetic rows populate text/status/date/progress; user/image/file fields are intentionally empty until safe runtime values are added during manual testing.
