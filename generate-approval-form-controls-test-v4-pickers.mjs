...
const outAppPath = "approval-form-controls-test-v4-app-def.json";
const outFormDefPath = "approval-form-controls-test-v4-approval-form-def.json";
const outListDefPath = "approval-form-controls-test-v4-list-def.json";
const outReportPath = "approval-form-controls-test-v4-generation-report.json";
...
const app = JSON.parse(JSON.stringify(source).replaceAll("427", family).replaceAll("DSX", formKey).replaceAll("dsv-", "afc4-"));
      ...
      container("Main", {
        ...
        container("Content", {
          ...
          container("Page header", cardAttrs(), [
            textControl("Approval Form Controls Test v4", "Dashboard title", "h3-bold"),
            paragraph("A focused proof package for picker native approval form controls learned from AI Training.", "Dashboard description")
          ])
        ])
      ])
    ],
...
function approvalControl(key, type, label, attrs = {}, readonly = false, pageReview = false, extra = {}) {
  const item = {
    id: `afc4-control-${key}-${pageReview ? "review" : "submit"}`,
    binding: key,
    type,
    label,
    attrs,
...
function makeApprovalPage(title, review = false) {
  ...
  const mainControls = [
    ...
    approvalControl("SelectedUser", "identity-picker", "Selected User", {
  ...
  const longControls = [
    approvalControl("DepartmentPicker", "organization-picker", "Department Picker", {
      placeholder: "Select a department if available",
      multiple: false,
      "metadata-treeselect": true
    }, readonly, review),
    approvalControl("CostCenterPicker", "cost-center-picker", "Cost Center Picker", {
      multiple: false
    }, readonly, review),
    approvalControl("LocationPicker", "location-picker", "Location Picker", {}, readonly, review),
    approvalControl("Notes", "textarea", "Notes", { edit: { fhlay: "auto", textarea_minrows: 3 }, placeholder: "Optional context for this picker test" }, readonly, review),
      ...
      container("Main", {
        ...
        container("Content", {
          ...
          container("Form body", cardAttrs(), [
            textControl(title, `${title} heading`, "h4-medium"),
            paragraph(review ? "Review the submitted control values and approve or reject the request." : "Submit picker controls generated from the AI Training control study.", `${title} helper`),
            flexGrid(mainControls, "Picker requester grid"),
            longSection(longControls, "People organization picker controls and notes")
          ]),
          container("Form bottom", {
            style: { gap: [null, "--sp--s200"], direction: [null, "column"] }
          }, [
            { id: `afc4-control-panel-${review ? "review" : "submit"}`, type: "workflowControlPanel", label: "Action Panel", attrs: { "show-task-panel": true, rejectValidation: true, align: "center" }, nv_label: "Action panel" },
            { id: `afc4-flow-history-${review ? "review" : "submit"}`, type: "workflowHistory", label: "Flow History", attrs: { "show-history": true }, nv_label: "Flow history" }
          ])
        ])
      ])
    ],
...
const fields = [
  ...
  ["Text3", "Picker Proof Scope", "PickerProofScope", "Text", "input", { required: false }],
...
baseList.ListModel.Title = "Picker Test Requests";
baseList.ListModel.Description = "Persisted records created by the Approval Form Controls Test v4 workflow.";
...
baseList.ListDatas = {
  [localId(2, "0000000011001")]: {
    ...
    Title: "Existing picker sample",
    Text1: "Current User",
    Text2: "Seed row for picker control testing.",
    Text3: "Picker controls render in workflow form; direct picker persistence deferred.",
    Text4: ""
  }
};
...
data.Item.ListModel.Title = "Approval Form Controls Test v4";
data.Item.ListModel.Description = "Small app to prove picker approval form controls from the AI Training study.";
...
data.Item.ListModel.LayoutView = JSON.stringify({
  ...
  sort: [
    ...
    { AppID: appId, ListID: requestListId, ListSetID: rootId, Type: 1, IsHidden: false, Title: "Picker Test Requests", Icon: "fa-regular fa-list-check" },
    { AppID: "41", Title: "Submit Pickers Test", ListID: formKey, ListSetID: rootId, Type: 105, Icon: "fa-regular fa-paper-plane" }
  ],
  attrs: {
...
form.Name = "Approval Form Controls Test v4";
form.Key = formKey;
form.Description = "Approval form with picker controls learned from AI Training.";
form.ListID = 0;
...
def.defkey = formKey;
def.name = "Approval Form Controls Test v4";
def.title = "Approval Form Controls Test v4";
...
def.listSet = rootId;
def.listInfo = { ListID: requestListId, Title: "Picker Test Requests" };
def.pageurls[0].id = uuid();
def.pageurls[0].title = "Submit Pickers Test";
def.pageurls[0].type = 1;
def.pageurls[0].formdef = makeApprovalPage("Submit Pickers Test", false);
def.pageurls[0].formdef.id = def.pageurls[0].id;
def.pageurls[1].id = uuid();
def.pageurls[1].title = "Review Pickers Test";
def.pageurls[1].type = 2;
def.pageurls[1].formdef = makeApprovalPage("Review Pickers Test", true);
def.pageurls[1].formdef.id = def.pageurls[1].id;
def.variables.basic = [
  { idx: "afc4-var-request-title", id: "RequestTitle", name: "Request Title", type: "text", editable: true },
  { idx: "afc4-var-requester", id: "Requester", name: "Requester", type: "user", editable: true },
  { idx: "afc4-var-selected-user", id: "SelectedUser", name: "Selected User", type: "user", editable: true },
  { idx: "afc4-var-department-picker", id: "DepartmentPicker", name: "Department Picker", type: "groupselect", editable: true },
  { idx: "afc4-var-cost-center-picker", id: "CostCenterPicker", name: "Cost Center Picker", type: "costcenter", editable: true },
  { idx: "afc4-var-location-picker", id: "LocationPicker", name: "Location Picker", type: "location", editable: true },
  { idx: "afc4-var-notes", id: "Notes", name: "Notes", type: "text", editable: true },
  { idx: "afc4-var-decision-notes", id: "DecisionNotes", name: "Decision Notes", type: "text", editable: true }
];
...
const startNodeId = "afc4-node-start-0001";
const reviewNodeId = "afc4-node-review-0002";
const persistNodeId = "afc4-node-persist-0003";
const endNodeId = "afc4-node-end-0004";
const rejectNodeId = "afc4-node-reject-0005";
const flowStartReview = "afc4-flow-0001";
const flowApproved = "afc4-flow-0002";
const flowRejected = "afc4-flow-0003";
const flowPersistEnd = "afc4-flow-0004";
...
def.childshapes = [
  ...
  workflowNode(persistNodeId, "ContentList", {
    ...
    listdatas: [
      ...
      { Per: "0", Columns: "Text3", Data: "Picker controls submitted in workflow form; direct picker persistence deferred." },
  ...
  workflowFlow(flowApproved, reviewNodeId, persistNodeId, {
    ...
    conditioninfo: [{ key: "afc4-cond-approved", pre: "and", left: taskOutcomeButton(reviewNodeId, "Reviewer Approval"), op: "s.=", right: outcomeValueButton("Approved") }]
  }),
  workflowFlow(flowRejected, reviewNodeId, rejectNodeId, {
    name: "Reviewer Rejected",
    documentation: "Rejected",
    conditioninfo: [{ key: "afc4-cond-rejected", pre: "and", left: taskOutcomeButton(reviewNodeId, "Reviewer Approval"), op: "s.=", right: outcomeValueButton("Rejected") }]
  }),
...
app.Title = "Approval Form Controls Test v4";
app.Description = "Small generated app proving picker native approval form controls.";
app.IconUrl = iconUrl;
...
fs.writeFileSync(outReportPath, `${JSON.stringify({
  ...
  runtimeSensitiveControlsIncludedForProof: ["organization-picker", "location-picker", "cost-center-picker"],
  infrastructureControlsNotPartOfStageProof: ["input", "textarea", "identity-picker", "workflowControlPanel", "workflowHistory"],
  deferredControls: ["metadata", "mutiple-metadata", "lookup-list", "list/sublist", "signer", "tag"],
  persistencePolicy: "Picker variables are rendered and submitted through workflow form controls. ContentList persists safe text summary fields only until direct picker data-list persistence is proven.",
  resources: {
    dataLists: ["Picker Test Requests"],
    approvalForms: ["Approval Form Controls Test v4"],
    dashboards: ["Overview"]
  }
}, null, 2)}\n`);
