...
const sourceResourcePath = "service-desk-pro-dashboard-stage-b-or-c-resource.json";
const outputAppDefPath = "service-desk-pro-dashboard-stage-d-app-def.json";
const outputResourcePath = "service-desk-pro-dashboard-stage-d-resource.json";
const outputReportPath = "service-desk-pro-dashboard-stage-d-generation-report.json";

const oldRootId = "2470010000000000000";
const oldDashboardId = "2470010000000000001";
const rootId = "2480010000000000000";
const dashboardId = "2480010000000000001";
const supportTicketsListId = "2480020000000001000";
const supportTicketsViewId = "2480020000000001801";
const generatedAt = "2026-05-13 00:45:00";
...
function fieldId(index) {
  return `248002000000000${String(1000 + index).padStart(4, "0")}`;
}

function rowId(index) {
  return `248002000000001${String(1000 + index).padStart(4, "0")}`;
}
...
data.Item.ListModel.Title = "Service Desk Pro Dashboard Stage D";
data.Item.ListModel.Description = "Service Desk Pro Executive Dashboard rebuild with static dashboard and one local Support Tickets source list.";
...
function walk(node) {
  ...
  if (node.type === "text-editor" && typeof node.attrs?.value === "string") {
    node.attrs.value = "<p>Support Tickets source list is now included locally. Data-bound summaries and filters remain paused until the list import/query path is proven.</p>";
  }
...
fs.writeFileSync(outputReportPath, `${JSON.stringify({
  ...
  includedPatterns: [
    "Stage C static Executive Dashboard",
  ...
  intentionallyExcluded: [
    "dashboard exts",
    "summary/chart/table data binding",
