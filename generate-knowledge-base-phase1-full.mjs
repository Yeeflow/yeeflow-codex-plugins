...
const outputAppDefPath = "knowledge-base-phase1-full-app-def.json";
const outputResourcePath = "knowledge-base-phase1-full-resource.json";
const outputReportPath = "knowledge-base-phase1-full-generation-report.json";

const oldFamily = "267";
const family = "281";
const generatedAt = "2026-05-14 08:20:00";
const appTitle = "Knowledge Base Phase 1 Full";
const appDescription = "Phase 1 generated Knowledge Base app with local categories, sections, articles, dashboard Collections, and safe category lookup isolation.";
...
const articleLibraryDashboardId = `${family}0010000000000002`;
const articlesListId = `${family}0020000000001000`;
const categoriesListId = `${family}0030000000001000`;
const sectionsListId = `${family}0040000000001000`;
...
function badgeField(fieldName, prefix, color = "#2563eb", extra = {}) {
...
function articleCollection() {
  const item = control("container", "Container", {
    ...
    badgeField("Text5", "Section:", "#475569", { nv_label: "Article section label" }),
    badgeField("Text6", "Status:", "#15803d", { nv_label: "Article status label" }),
    dynamicField("Datetime1", {
      prefix: "Updated:",
      item_style: { ty: [null, "s-regular"], normal: { color: "var(--c--neutral-dark-hover)" } }
    }, { nv_label: "Article updated date" })
  ...
  return control("collection", "Collection", {
    data: {
      ...
      fulltext: [
        {
          fields: ["Title", "Text1", "Text2", "Text5", "Text6", "Text7"],
          value: [
            {
              exprType: "variable",
              valueType: "string",
...
function articleLibraryCollection() {
  const item = control("container", "Container", {
    style: { gap: [null, "--sp--s100"] },
    common: {
      ...
      border: {
        normal: {
          ...
          width: [null, { top: 1, right: 1, bottom: 1, left: 1 }],
          color: "var(--c--neutral-light-active)",
          radius: [null, { top: "--sp--s150", right: "--sp--s150", bottom: "--sp--s150", left: "--sp--s150" }]
        }
      }
    }
  }, [
    dynamicField("Title", {
      item_style: { ty: { size: [null, "--sp--s200"], wei: "600" }, normal: { color: "var(--c--text)" } }
    }, { nv_label: "Library article title" }),
    dynamicField("Text1", {
      item_style: { ty: [null, "s-regular"], normal: { color: "var(--c--neutral-dark-hover)" } }
    }, { nv_label: "Library article summary" }),
    badgeField("Text2", "Category:", "var(--c--primary)", { nv_label: "Library article category" }),
    badgeField("Text5", "Section:", "#475569", { nv_label: "Library article section" }),
    badgeField("Text6", "Status:", "#15803d", { nv_label: "Library article status" })
  ], { nv_label: "Article library row card" });
  ...
  return control("collection", "Collection", {
    ...
    layout: {
      cg: [null, 16],
      rg: [null, 16],
      cp: [null, { top: "--sp--s150", right: "--sp--s150", bottom: "--sp--s150", left: "--sp--s150" }]
    }
  }, [item], { nv_label: "Article library collection" });
}
...
function categoryCollection() {
  const item = control("container", "Container", {
    ...
    badgeField("Decimal1", "Order:", "#64748b", { nv_label: "Category order" })
...
function sectionCollection() {
  const item = control("container", "Container", {
    style: { gap: [null, "--sp--s075"] },
    common: {
      padding: [null, { top: "--sp--s225", right: "--sp--s225", bottom: "--sp--s225", left: "--sp--s225" }],
      background: { normal: { type: "classic", classic: { color: "#f8fafc" } } },
      border: {
        normal: {
          type: "1",
    ...
    dynamicField("Title", {
      item_style: { ty: { size: [null, "--sp--s175"], wei: "600" }, normal: { color: "var(--c--text)" } }
    }, { nv_label: "Section title" }),
    badgeField("Text1", "Category:", "var(--c--primary)", { nv_label: "Section category label" }),
    dynamicField("Text2", {
      item_style: { ty: [null, "s-regular"], normal: { color: "var(--c--neutral-dark-hover)" } }
    }, { nv_label: "Section description" })
  ], { nv_label: "Section topic item" });

  return control("collection", "Collection", {
    data: {
      list: { AppID: appId, ListID: sectionsListId, Type: 1, Title: "Sections", ListSetID: rootId },
      limit: false,
      disv: false,
      ps: 12,
      filter: [],
  ...
  }, [item], { nv_label: "Section topic collection" });
}
    ...
    children: [
      control("container", "Container", {
        ...
        control("container", "Container", {
          ...
          heading("Explore sections", "h3-bold"),
          sectionCollection()
        ], { nv_label: "Browse sections section" })
  ...
  return {
    children: [
      control("container", "Container", {
        ...
        style: { gap: [null, "--sp--s400"] }
      }, [
        control("container", "Container", {
          style: { gap: [null, "--sp--s150"] }
        }, [
          heading("Article Library", "h1-bold"),
          text("<p>Review published and draft knowledge articles by category, section, and status.</p>")
        ], { nv_label: "Article library header" }),
        control("container", "Container", {
          style: { gap: [null, "--sp--s200"] }
        }, [
          heading("All articles", "h3-bold"),
          articleLibraryCollection()
        ], { nv_label: "Article library section" })
      ], { nv_label: "Article Library page" })
    ],
    attrs: {},
...
function makeBitField(listId, fieldId, fieldName, displayName, rules = {}, overrides = {}) {
  return makeField(listId, fieldId, fieldName, displayName, "switch", rules, {
    FieldType: "Bit",
    FieldIndex: Number(fieldName.match(/\d+$/)?.[0] || 1),
    IsSort: false,
    IsIndex: false,
    IsFilter: false,
    IsIndexCreated: false,
...
function makeDateField(listId, fieldId, fieldName, displayName, rules = {}, overrides = {}) {
  return makeField(listId, fieldId, fieldName, displayName, "datepicker", rules, {
    FieldType: "Datetime",
    FieldIndex: Number(fieldName.match(/\d+$/)?.[0] || 1),
    IsSort: false,
    IsIndex: false,
...
const categoryFields = [
  ...
  makeField(categoriesListId, `${family}0030000000001004`, "Text2", "Accent", "input", { placeholder: "Visual accent label or color" })
];

const sectionFields = [
  makeField(sectionsListId, `${family}0040000000001001`, "Title", "Section Name", "input", { required: true }),
  makeField(sectionsListId, `${family}0040000000001002`, "Text1", "Category Label", "input", { placeholder: "Parent category label" }),
  makeField(sectionsListId, `${family}0040000000001003`, "Text2", "Description", "textarea", { placeholder: "Describe this section" }),
  makeDecimalField(sectionsListId, `${family}0040000000001004`, "Decimal1", "Order", { number_min: 1, number_step: 1, "rounded-to": "0" }, { FieldIndex: 1 })
];
...
const articleFields = [
  ...
  makeField(articlesListId, `${family}0020000000001006`, "Text5", "Section Label", "input", { placeholder: "Display section" }),
  makeField(articlesListId, `${family}0020000000001007`, "Text6", "Status", "radio", {
    options: [
  ...
  makeBitField(articlesListId, `${family}0020000000001008`, "Bit1", "Featured", { defaultValue: false }),
  makeDateField(articlesListId, `${family}0020000000001009`, "Datetime1", "Updated Date", { showtime: false, dateformat: "0" }),
  makeField(articlesListId, `${family}0020000000001010`, "Text7", "Body Excerpt", "textarea", { placeholder: "Article body excerpt" })
];

const categoryRows = [
  row(`${family}0030000000011001`, { Title: "Getting Started", Text1: "Introductory guides for new users and makers.", Decimal1: "1", Text2: "blue" }),
  row(`${family}0030000000011002`, { Title: "Application Builder", Text1: "Patterns for lists, dashboards, forms, and navigation.", Decimal1: "3", Text2: "purple" }),
  row(`${family}0030000000011003`, { Title: "Operations", Text1: "Practical administration, permissions, and governance notes.", Decimal1: "2", Text2: "green" })
];

const sectionRows = [
  row(`${family}0040000000011001`, { Title: "Getting Started", Text1: "Getting Started", Text2: "Workspace basics and first steps.", Decimal1: "1" }),
  row(`${family}0040000000011002`, { Title: "Data and dashboards", Text1: "Application Builder", Text2: "List, dashboard, and navigation design patterns.", Decimal1: "2" }),
  row(`${family}0040000000011003`, { Title: "Governance", Text1: "Operations", Text2: "Ownership, review cadence, permissions, and maintenance.", Decimal1: "3" })
];
...
const articleRows = [
  row(`${family}0020000000011001`, {
    ...
    Text5: "Getting Started",
    Text6: "Published",
    Bit1: "1",
    Datetime1: "2026-05-01T09:00:00",
    Text7: "Start with a simple app shell, define the core data lists, then add dashboard navigation after list behavior is stable.",
    Text3: "",
    Text4: ""
  }),
  row(`${family}0020000000011002`, {
    ...
    Text5: "Data and dashboards",
    Text6: "Published",
    Bit1: "1",
    Datetime1: "2026-05-04T09:00:00",
    Text7: "Keep the native Title field for article names, use summary text for scanning, and keep category labels stable for dashboards.",
    Text3: "",
    Text4: ""
  }),
  row(`${family}0020000000011003`, {
    ...
    Text6: "Review",
    Bit1: "0",
    Datetime1: "2026-05-08T09:00:00",
    Text7: "Assign owners and review windows before the article library grows, so outdated guidance can be found and refreshed.",
    Text3: "",
    Text4: ""
  })
];
...
const articleLibraryDashboard = clone(dashboard);
articleLibraryDashboard.Title = "Article Library";
articleLibraryDashboard.LayoutID = articleLibraryDashboardId;
articleLibraryDashboard.LayoutInResources = [{ ID: articleLibraryDashboardId, RefId: articleLibraryDashboardId, Resource: JSON.stringify(articleLibraryPage()) }];
data.Item.Layouts = [dashboard, articleLibraryDashboard];

data.Childs = [
  makeList(categoriesListId, "Categories", categoryFields, categoryRows, {
    visibleFieldNames: ["Title", "Decimal1", "Text2"],
    preSystemFieldNames: ["Title"],
    postSystemFieldNames: ["Decimal1", "Text2"]
  }),
  makeList(sectionsListId, "Sections", sectionFields, sectionRows, {
    visibleFieldNames: ["Title", "Text1", "Decimal1"],
    preSystemFieldNames: ["Title", "Text1"],
    postSystemFieldNames: ["Decimal1"]
  }),
  makeList(articlesListId, "Articles", articleFields, articleRows, {
    visibleFieldNames: ["Title", "Text2", "Text5", "Text6", "Datetime1", "Text3"],
    preSystemFieldNames: ["Title", "Text2", "Text5", "Text6", "Datetime1", "Text4"],
...
root.LayoutView = JSON.stringify({
  ...
  sort: [
    ...
    { AppID: appId, ListID: articleLibraryDashboardId, ListSetID: rootId, Type: 103, Title: "Article Library", Icon: "fa-regular fa-list", DisplayName: "Article Library" },
    ...
    { AppID: appId, ListID: sectionsListId, ListSetID: rootId, Type: 1, Title: "Sections", Icon: "fa-regular fa-layer-group", DisplayName: "Sections" },
...
const replaceIds = [
  rootId,
  dashboardId,
  articleLibraryDashboardId,
  categoriesListId,
  ...categoryFields.map((field) => field.FieldID),
  `${categoriesListId.slice(0, 6)}0000000001801`,
  ...categoryRows.map((item) => item.ListDataID),
  sectionsListId,
  ...sectionFields.map((field) => field.FieldID),
  `${sectionsListId.slice(0, 6)}0000000001801`,
  ...sectionRows.map((item) => item.ListDataID),
  articlesListId,
...
fs.writeFileSync(outputReportPath, `${JSON.stringify({
  ...
  includedPatterns: [
    "root app shell",
    "three local data lists",
    "category label text metadata and sample values",
    "Categories Decimal1 Order metadata and sample values",
    "Sections plain topic list metadata and sample values",
    "article section/status/featured/updated/body excerpt metadata",
